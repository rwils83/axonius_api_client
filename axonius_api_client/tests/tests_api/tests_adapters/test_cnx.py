...
class TestCnxPublic(TestCnxBase):
    ...
    def test_get_by_adapter_badnode(self, apiobj):
        with pytest.raises(NotFoundError):
            apiobj.cnx.get_by_adapter(adapter_name=CSV_ADAPTER,
    ...
    def test_update_cnx(self, apiobj):
        ...
        cnx_update = apiobj.cnx.update_cnx(cnx_update=cnx,
                                           connection_label=new_label)
    ...
    def test_update_cnx_error(self, apiobj):
        ...
        cnx_reset = apiobj.cnx.update_cnx(cnx_update=exc.value.cnx_new,
                                          https_proxy=old_https_proxy)
    ...
    def test_add_remove(self, apiobj, csv_file_path):
        ...
        cnx_added = apiobj.cnx.add(adapter_name=CSV_ADAPTER,
                                   adapter_node=DEFAULT_NODE,
                                   **config)

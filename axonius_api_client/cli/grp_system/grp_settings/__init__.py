# -*- coding: utf-8 -*-
"""Command line interface for Axonius API Client."""
import click

from . import cmd_get
from . import cmd_get_section
from . import cmd_get_subsection
from . import cmd_update_section
from . import cmd_update_section_from_json
from . import cmd_update_subsection
from . import cmd_update_subsection_from_json
from ...context import AliasedGroup


@click.group(cls=AliasedGroup)
def settings_core():
    """Group: Global Settings."""


@click.group(cls=AliasedGroup)
def settings_lifecycle():
    """Group: Lifecycle Settings."""


@click.group(cls=AliasedGroup)
def settings_gui():
    """Group: GUI Settings."""


CMDS = [
    cmd_get,
    cmd_get_section,
    cmd_get_subsection,
    cmd_update_section,
    cmd_update_subsection,
    cmd_update_section_from_json,
    cmd_update_subsection_from_json,
]

for cmd in CMDS:
    settings_core.add_command(cmd.cmd)
    settings_lifecycle.add_command(cmd.cmd)
    settings_gui.add_command(cmd.cmd)
